...
class AnimanaApiClient {
  ...
  async getClients(query?: string, page = 1, limit = 20) {
    return api.get<ApiResponse<AnimanaOwner[]>>(
      `/animana/clients?query=${query || ""}&page=${page}&limit=${limit}`
    );
  }
  ...
  async updateClient(clientId: string, clientData: Partial<AnimanaOwner>) {
    return api.put<ApiResponse<AnimanaOwner>>(
      `/animana/clients/${clientId}`,
      clientData
    );
  }
  ...
  async getPatientById(patientId: string) {
    return api.get<ApiResponse<AnimanaPatient>>(
      `/animana/patients/${patientId}`
    );
  }

  async createPatient(patientData: Omit<AnimanaPatient, "id">) {
    return api.post<ApiResponse<AnimanaPatient>>(
      "/animana/patients",
      patientData
    );
  }
  ...
  async updatePatient(patientId: string, patientData: Partial<AnimanaPatient>) {
    return api.put<ApiResponse<AnimanaPatient>>(
      `/animana/patients/${patientId}`,
      patientData
    );
  }
  ...
  async getMedicalRecords(patientId: string, page = 1, limit = 20) {
    return api.get<ApiResponse<MedicalRecord[]>>(
      `/animana/patients/${patientId}/records?page=${page}&limit=${limit}`
    );
  }
  ...
  async createMedicalRecord(
    ...
    return api.post<ApiResponse<MedicalRecord>>(
      `/animana/patients/${patientId}/records`,
      recordData
    );
  }
  ...
  async getAppointmentById(appointmentId: string) {
    return api.get<ApiResponse<AnimanaAppointment>>(
      `/animana/appointments/${appointmentId}`
    );
  }

  async createAppointment(appointmentData: Omit<AnimanaAppointment, "id">) {
    return api.post<ApiResponse<AnimanaAppointment>>(
      "/animana/appointments",
      appointmentData
    );
  }
  ...
  async updateAppointment(
    ...
    appointmentData: Partial<AnimanaAppointment>
  ) {
    return api.put<ApiResponse<AnimanaAppointment>>(
      `/animana/appointments/${appointmentId}`,
      appointmentData
    );
  }
  ...
  async cancelAppointment(appointmentId: string, reason: string) {
    return api.post<ApiResponse<{ success: boolean }>>(
      `/animana/appointments/${appointmentId}/cancel`,
      { reason }
    );
  }
